...
def speed_reward(params):
    ...
    heading = params['heading']
    speed = params['speed']
    max_waypoints = len(waypoints) - 1

    prev_point_index = closest_waypoints[0]
    next_point_index = closest_waypoints[1]
    prev_point = optimal_waypoints[prev_point_index]
    next_point = optimal_waypoints[next_point_index]
    second_point = optimal_waypoints[min(closest_waypoints[1] + 1, max_waypoints)]
    fifth_point = optimal_waypoints[min(closest_waypoints[1] + 4, max_waypoints)]

    first_track_direction = math.atan2(next_point[1] - prev_point[1], next_point[0] - prev_point[0]) # X
    second_track_direction = math.atan2(second_point[1] - prev_point[1], second_point[0] - prev_point[0]) # Y
    fifth_track_direction = math.atan2(fifth_point[1] - prev_point[1], fifth_point[0] - prev_point[0]) # Z

    # Convert to degree
    first_track_direction = math.degrees(first_track_direction)
    second_track_direction =  math.degrees(second_track_direction)
    fifth_track_direction = math.degrees(fifth_track_direction)

    # Find difference
    second_direction_diff = degree_diff(first_track_direction, second_track_direction)
...
def degree_diff(curr, next):
    ...
    if predict_direction_diff > 180:
        predict_direction_diff = 360 - predict_direction_diff
    return predict_direction_diff
...
def reward_function(params):
    ...
    if optimal_heading_diff > 30:
        ...
        reward = angle_reward * 1.7
    elif optimal_heading_diff >= 0:
        reward = angle_reward * 2.2
    
    reward += speed_reward(params)
